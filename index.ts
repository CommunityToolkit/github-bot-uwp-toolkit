...
const getAllGitHubIssuesRecursively = (headers: any, repoOwner: string, repoName: string, afterCursor: string, callback: (issues: any[]) => any) => {
    performGitHubGraphqlRequest(headers, {
        ...
        if (response.data.repository.issues.pageInfo.hasNextPage) {
            getAllGitHubIssuesRecursively(headers, repoOwner, repoName, response.data.repository.issues.pageInfo.endCursor, (issues) => {
                callback(issues.concat(response.data.repository.issues.edges));
            });
        } else {
            callback(response.data.repository.issues.edges);
        }
    });
}
...
const detectIfNoResponseFromCommunity = (githubApiHeaders: any, issue: any, exclusiveLabels: string[]) => {
    // check if there is only one user who write a message (the creator of the message)
    const loginsOfAuthors: string[] = distinct(issue.node.commentAuthors.edges.map(edge => edge.node.author.login));
    const issueHasNoResponse = distinct(loginsOfAuthors.filter(c => c !== issue.node.author.login)).length === 0;

    if (issueHasNoResponse) {
        // check if the issue contains an exclusive labels (only issues without those labels are usable)
        const containsExclusiveLabels = issue.node.labels.edges.map(edge => edge.node).filter(label => {
        ...
        if (!containsExclusiveLabels) {
            ...
            if (new Date(issue.node.createdAt) < addDays(today, -7)) {
                // send a message with a ping to the team
                commentGitHubIssue(githubApiHeaders, issue.node.id, `No response from the community. ping @nmetulev`);
            }
        }
    }
}
...
const sendReminderAfterAPeriodOfTime = (githubApiHeaders: any, botUsername: string, issue: any, exclusiveLabels: string[]) => {
    // check if at least two users write a message (one user other than the author)
    const loginsOfAuthors: string[] = distinct(issue.node.commentAuthors.edges.map(edge => edge.node.author.login));
    const issueHasResponse = distinct(loginsOfAuthors.filter(c => c !== issue.node.author.login)).length > 0;
    ...
    if (issueHasResponse) {
        ...
        if (!containsExclusiveLabels) {
            ...
            const lastComment = issue.node.lastComment.edges[0];
            const today = new Date();
            ...
            if (lastComment && new Date(lastComment.node.updatedAt) < addDays(today, -7)) {
                ...
                const lastTwoMessages = issue.node.lastTwoComments.edges.map(edge => edge.node);
                ...
                if (issue.node.comments.totalCount < 3 || reminderMessagesFromBot.length < 2) {
...
const closeInactiveIssueAfterThreeSuccessiveAlerts = (githubApiHeaders: any, botUsername: string, issue: any, exclusiveLabels: string[]) => {
    // check if at least two users write a message (one user other than the author)
    const loginsOfAuthors: string[] = distinct(issue.node.commentAuthors.edges.map(edge => edge.node.author.login));
    const issueHasResponse = distinct(loginsOfAuthors.filter(c => c !== issue.node.author.login)).length > 0;
    ...
    if (issueHasResponse) {
        ...
        if (!containsExclusiveLabels) {
            ...
            if (lastComment && new Date(lastComment.updatedAt) < addDays(today, -7)) {
                ...
                if (reminderMessagesFromBot.length === 2) {
                    ...
                    closeGitHubIssue(githubApiHeaders, issue.node.id);
                }
            }
        }
    }
